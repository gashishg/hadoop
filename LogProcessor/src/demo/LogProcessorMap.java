...
public class LogProcessorMap extends
	...
	private static Logger log = Logger.getLogger(LogProcessorMap.class);

	static {
		log.info("--- Inside LogProcessorMap Ashish----");
	}
	 ...
	 * Path[] localCachePath;
	 * 
	 * public void setup(Context context) throws IOException{ Configuration conf
	 * = context.getConfiguration(); localCachePath =
	 * DistributedCache.getLocalCacheArchives(conf);
	 * 
	 * File lookupDbDir = new File("ip2locationdb"); String[] children =
	 * lookupDbDir.list();
	 * 
	 * if (children == null) {
	 * System.out.println("Cached archive directory is empty!!"); } else { for
	 * (int i=0; i<children.length; i++) { System.out.println(children[i]); } }
	 * }
	 */
